...
static void getValuesFromIntAttribute(DenseIntElementsAttr attr,
  ...
  for (auto val : attr.getValues<APInt>())
    arrayValues.push_back(val.getSExtValue());
}
...
static Value applyPad(Location loc, Value input, ArrayRef<int64_t> pad,
  ...
  if (llvm::all_of(pad, [](int64_t p) { return p == 0; })) return input;
  ...
  for (unsigned i = 0, s = inputShape.size(); i < s; i++) {
...
struct NormalConvOpOnTensorsConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    if (padding && !isSplatValue(*op.padding(), 0)) {
      if (rank != 4)
        return rewriter.notifyMatchFailure(op, "padding expected 2d input.");
      ...
      // Create conv op using the padded input.
      auto convOp = rewriter.create<linalg::Conv2DNhwcHwcfOp>(
          loc, result_type, ValueRange{input, filter}, ValueRange{zero_tensor},
          strides, dilations, PruneAttributeList(op));
      rewriter.replaceOp(op, convOp.getOperation()->getResults());
      return success();
    }
