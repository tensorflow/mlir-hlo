...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<1x2x3xf64> {mhlo.layout_mode = "default"}, tensor<1xf64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<[[[-1.1429763551803362, 3.7534087819203261, -1.9174025218327606], [-5.0596952963106343, 2.7799506328403263, 0.86546961799467437]]]> : tensor<1x2x3xf64>
    %cst_0 = stablehlo.constant dense<0.47280139191719139> : tensor<1xf64>
    return %cst, %cst_0 : tensor<1x2x3xf64>, tensor<1xf64>
  }
  func.func private @expected() -> (tensor<1x2x3xf64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<[[[-1.1429763551803362, 3.7534087819203261, -1.9174025218327606], [-5.0596952963106343, 2.7799506328403263, 1.3382710099118658]]]> : tensor<1x2x3xf64>
