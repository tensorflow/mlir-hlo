...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<1x2xui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[3, 2]]> : tensor<1x2xui8>
    return %c : tensor<1x2xui8>
  }
  func.func private @expected() -> (tensor<2xui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[3, 2]> : tensor<2xui8>
