...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3xi32> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:3 = call @inputs() : () -> (tensor<3xi32>, tensor<1xi32>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_update_slice %0#0, %0#1, %6 : (tensor<3xi32>, tensor<1xi32>, tensor<i64>) -> tensor<3xi32>
    stablehlo.custom_call @check.expect_eq(%7, %1) {has_side_effect = true} : (tensor<3xi32>, tensor<3xi32>) -> ()
    return %7 : tensor<3xi32>
  }
  func.func private @inputs() -> (tensor<3xi32> {mhlo.layout_mode = "default"}, tensor<1xi32> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[-2, 4, -1]> : tensor<3xi32>
    %c_0 = stablehlo.constant dense<-1> : tensor<1xi32>
    %c_1 = stablehlo.constant dense<1> : tensor<1xi64>
    return %c, %c_0, %c_1 : tensor<3xi32>, tensor<1xi32>, tensor<1xi64>
  }
  func.func private @expected() -> (tensor<3xi32> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[-2, -1, -1]> : tensor<3xi32>
