...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xf32> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:2 = call @inputs() : () -> (tensor<5xf32>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_slice %0#0, %6, sizes = [1] : (tensor<5xf32>, tensor<i64>) -> tensor<1xf32>
    stablehlo.custom_call @check.expect_close(%7, %1) {has_side_effect = true} : (tensor<1xf32>, tensor<1xf32>) -> ()
    return %7 : tensor<1xf32>
  }
  func.func private @inputs() -> (tensor<5xf32> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<[-2.26375055, 5.11517334, 1.33605301, 5.75105953, -3.70299459]> : tensor<5xf32>
    %c = stablehlo.constant dense<-10> : tensor<1xi64>
    return %cst, %c : tensor<5xf32>, tensor<1xi64>
  }
  func.func private @expected() -> (tensor<1xf32> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<-2.26375055> : tensor<1xf32>
