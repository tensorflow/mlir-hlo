...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<1x2xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[-2, 3]]> : tensor<1x2xi8>
    return %c : tensor<1x2xi8>
  }
  func.func private @expected() -> (tensor<2xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[-2, 3]> : tensor<2xi8>
