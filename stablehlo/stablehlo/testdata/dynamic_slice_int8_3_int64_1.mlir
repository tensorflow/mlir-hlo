...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xi8> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:2 = call @inputs() : () -> (tensor<3xi8>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_slice %0#0, %6, sizes = [1] : (tensor<3xi8>, tensor<i64>) -> tensor<1xi8>
    stablehlo.custom_call @check.expect_eq(%7, %1) {has_side_effect = true} : (tensor<1xi8>, tensor<1xi8>) -> ()
    return %7 : tensor<1xi8>
  }
  func.func private @inputs() -> (tensor<3xi8> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[0, 3, 2]> : tensor<3xi8>
    %c_0 = stablehlo.constant dense<1> : tensor<1xi64>
    return %c, %c_0 : tensor<3xi8>, tensor<1xi64>
  }
  func.func private @expected() -> (tensor<1xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<3> : tensor<1xi8>
