...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xui8> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %0:2 = call @inputs() : () -> (tensor<1xui8>, tensor<ui8>)
  ...
  func.func private @inputs() -> (tensor<1xui8> {mhlo.layout_mode = "default"}, tensor<ui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<0> : tensor<1xui8>
    %c_0 = stablehlo.constant dense<3> : tensor<ui8>
    return %c, %c_0 : tensor<1xui8>, tensor<ui8>
  }
