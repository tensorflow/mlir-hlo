...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<1x2x3xi8> {mhlo.layout_mode = "default"}, tensor<1x3xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[7, 4, -2], [0, 2, 0]]]> : tensor<1x2x3xi8>
    %c_0 = stablehlo.constant dense<[[-3, 0, 0]]> : tensor<1x3xi8>
    return %c, %c_0 : tensor<1x2x3xi8>, tensor<1x3xi8>
  }
  func.func private @expected() -> (tensor<1x2x3xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[7, 4, -2], [-3, 0, 0]]]> : tensor<1x2x3xi8>
