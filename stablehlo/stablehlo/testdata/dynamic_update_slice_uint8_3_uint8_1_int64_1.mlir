...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3xui8> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:3 = call @inputs() : () -> (tensor<3xui8>, tensor<1xui8>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_update_slice %0#0, %0#1, %6 : (tensor<3xui8>, tensor<1xui8>, tensor<i64>) -> tensor<3xui8>
    stablehlo.custom_call @check.expect_eq(%7, %1) {has_side_effect = true} : (tensor<3xui8>, tensor<3xui8>) -> ()
    return %7 : tensor<3xui8>
  }
  func.func private @inputs() -> (tensor<3xui8> {mhlo.layout_mode = "default"}, tensor<1xui8> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[2, 4, 4]> : tensor<3xui8>
    %c_0 = stablehlo.constant dense<1> : tensor<1xui8>
    %c_1 = stablehlo.constant dense<1> : tensor<1xi64>
    return %c, %c_0, %c_1 : tensor<3xui8>, tensor<1xui8>, tensor<1xi64>
  }
  func.func private @expected() -> (tensor<3xui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[2, 1, 4]> : tensor<3xui8>
