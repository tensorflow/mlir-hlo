...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xi64> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %0:2 = call @inputs() : () -> (tensor<1xi64>, tensor<2xi64>)
  ...
  func.func private @inputs() -> (tensor<1xi64> {mhlo.layout_mode = "default"}, tensor<2xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<-1> : tensor<1xi64>
    %c_0 = stablehlo.constant dense<[-6, -2]> : tensor<2xi64>
    return %c, %c_0 : tensor<1xi64>, tensor<2xi64>
  }
  func.func private @expected() -> (tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<-12> : tensor<1xi64>
