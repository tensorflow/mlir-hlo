...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<4x2x3xi32> {mhlo.layout_mode = "default"}, tensor<2xi32> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[-2, 7, 0], [0, 0, 0]], [[-1, 1, -9], [-6, 2, 2]], [[8, -5, 4], [0, 3, 0]], [[0, 1, 3], [-1, -3, 2]]]> : tensor<4x2x3xi32>
    %c_0 = stablehlo.constant dense<[1, 0]> : tensor<2xi32>
    return %c, %c_0 : tensor<4x2x3xi32>, tensor<2xi32>
  }
  func.func private @expected() -> (tensor<4x2x3xi32> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[-2, 7, 0], [0, 0, 0]], [[-1, 1, -9], [-6, 2, 2]], [[8, -5, 4], [0, 3, 0]], [[0, 1, 1], [-1, -3, 0]]]> : tensor<4x2x3xi32>
