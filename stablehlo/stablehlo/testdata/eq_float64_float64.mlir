...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<i1> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %2 = stablehlo.compare  EQ, %0#0, %0#1,  FLOAT : (tensor<f64>, tensor<f64>) -> tensor<i1>
  ...
  func.func private @inputs() -> (tensor<f64> {mhlo.layout_mode = "default"}, tensor<f64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<4.2512048214004619> : tensor<f64>
    %cst_0 = stablehlo.constant dense<4.6022897951074313> : tensor<f64>
