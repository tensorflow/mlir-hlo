...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<f64> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %2 = call @"<lambda>"(%0) : (tensor<f64>) -> tensor<f64>
  ...
  func.func private @inputs() -> (tensor<f64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<0.024558972542420593> : tensor<f64>
    return %cst : tensor<f64>
  }
  func.func private @expected() -> (tensor<f64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<6.1163297886932108E-4> : tensor<f64>
    return %cst : tensor<f64>
  }
  func.func private @"<lambda>"(%arg0: tensor<f64> {mhlo.layout_mode = "default"}) -> (tensor<f64> {mhlo.layout_mode = "default"}) {
    ...
    %8 = call @_gamma(%7, %arg0) : (tensor<2xui32>, tensor<f64>) -> tensor<f64>
    return %8 : tensor<f64>
  }
  func.func private @_gamma(%arg0: tensor<2xui32> {mhlo.layout_mode = "default"}, %arg1: tensor<f64> {mhlo.layout_mode = "default"}) -> (tensor<f64> {mhlo.layout_mode = "default"}) {
    %0 = stablehlo.reshape %arg0 : (tensor<2xui32>) -> tensor<1x2xui32>
    %1 = call @_threefry_split(%0) : (tensor<1x2xui32>) -> tensor<1x1x2xui32>
    %2 = stablehlo.reshape %1 : (tensor<1x1x2xui32>) -> tensor<1x2xui32>
    %3 = stablehlo.reshape %arg1 : (tensor<f64>) -> tensor<1xf64>
    %cst = stablehlo.constant dense<0.000000e+00> : tensor<f64>
    %4 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<f64>) -> tensor<1xf64>
    %c = stablehlo.constant dense<0> : tensor<i64>
    %5:4 = stablehlo.while(%iterArg = %2, %iterArg_0 = %3, %iterArg_1 = %c, %iterArg_2 = %4) : tensor<1x2xui32>, tensor<1xf64>, tensor<i64>, tensor<1xf64>
     cond {
      %c_3 = stablehlo.constant dense<1> : tensor<i64>
      %7 = stablehlo.compare  LT, %iterArg_1, %c_3,  SIGNED : (tensor<i64>, tensor<i64>) -> tensor<i1>
      stablehlo.return %7 : tensor<i1>
    } do {
      %c_3 = stablehlo.constant dense<0> : tensor<i64>
      %7 = stablehlo.compare  LT, %iterArg_1, %c_3,  SIGNED : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %8 = stablehlo.convert %iterArg_1 : tensor<i64>
      %c_4 = stablehlo.constant dense<1> : tensor<i64>
      %9 = stablehlo.add %8, %c_4 : tensor<i64>
      %10 = stablehlo.select %7, %9, %iterArg_1 : tensor<i1>, tensor<i64>
      %c_5 = stablehlo.constant dense<0> : tensor<i64>
      %11 = stablehlo.dynamic_slice %iterArg, %10, %c_5, sizes = [1, 2] : (tensor<1x2xui32>, tensor<i64>, tensor<i64>) -> tensor<1x2xui32>
      %12 = stablehlo.reshape %11 : (tensor<1x2xui32>) -> tensor<2xui32>
      %c_6 = stablehlo.constant dense<0> : tensor<i64>
      %13 = stablehlo.compare  LT, %iterArg_1, %c_6,  SIGNED : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %14 = stablehlo.convert %iterArg_1 : tensor<i64>
      %c_7 = stablehlo.constant dense<1> : tensor<i64>
      %15 = stablehlo.add %14, %c_7 : tensor<i64>
      %16 = stablehlo.select %13, %15, %iterArg_1 : tensor<i1>, tensor<i64>
      %17 = stablehlo.dynamic_slice %iterArg_0, %16, sizes = [1] : (tensor<1xf64>, tensor<i64>) -> tensor<1xf64>
      %18 = stablehlo.reshape %17 : (tensor<1xf64>) -> tensor<f64>
      %19 = func.call @None_0(%12, %18) : (tensor<2xui32>, tensor<f64>) -> tensor<f64>
      %20 = stablehlo.broadcast_in_dim %19, dims = [] : (tensor<f64>) -> tensor<1xf64>
      %c_8 = stablehlo.constant dense<0> : tensor<i64>
      %21 = stablehlo.compare  LT, %iterArg_1, %c_8,  SIGNED : (tensor<i64>, tensor<i64>) -> tensor<i1>
      %22 = stablehlo.convert %iterArg_1 : tensor<i64>
      %c_9 = stablehlo.constant dense<1> : tensor<i64>
      %23 = stablehlo.add %22, %c_9 : tensor<i64>
      %24 = stablehlo.select %21, %23, %iterArg_1 : tensor<i1>, tensor<i64>
      %25 = stablehlo.dynamic_update_slice %iterArg_2, %20, %24 : (tensor<1xf64>, tensor<1xf64>, tensor<i64>) -> tensor<1xf64>
      %c_10 = stablehlo.constant dense<1> : tensor<i64>
      %26 = stablehlo.add %iterArg_1, %c_10 : tensor<i64>
      stablehlo.return %iterArg, %iterArg_0, %26, %25 : tensor<1x2xui32>, tensor<1xf64>, tensor<i64>, tensor<1xf64>
    }
    %6 = stablehlo.reshape %5#3 : (tensor<1xf64>) -> tensor<f64>
    return %6 : tensor<f64>
  }
