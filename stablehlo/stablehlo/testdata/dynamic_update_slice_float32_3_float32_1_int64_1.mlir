...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3xf32> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:3 = call @inputs() : () -> (tensor<3xf32>, tensor<1xf32>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_update_slice %0#0, %0#1, %6 : (tensor<3xf32>, tensor<1xf32>, tensor<i64>) -> tensor<3xf32>
    stablehlo.custom_call @check.expect_close(%7, %1) {has_side_effect = true} : (tensor<3xf32>, tensor<3xf32>) -> ()
    return %7 : tensor<3xf32>
  }
  func.func private @inputs() -> (tensor<3xf32> {mhlo.layout_mode = "default"}, tensor<1xf32> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<[1.21156347, 2.40181637, -1.9381783]> : tensor<3xf32>
    %cst_0 = stablehlo.constant dense<-4.01614714> : tensor<1xf32>
    %c = stablehlo.constant dense<1> : tensor<1xi64>
    return %cst, %cst_0, %c : tensor<3xf32>, tensor<1xf32>, tensor<1xi64>
  }
  func.func private @expected() -> (tensor<3xf32> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<[1.21156347, -4.01614714, -1.9381783]> : tensor<3xf32>
