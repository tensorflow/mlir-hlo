...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<i1> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %7 = stablehlo.compare  GT, %5, %6,  FLOAT : (tensor<f64>, tensor<f64>) -> tensor<i1>
    %8 = stablehlo.real %0#0 : (tensor<complex<f64>>) -> tensor<f64>
    %9 = stablehlo.real %0#1 : (tensor<complex<f64>>) -> tensor<f64>
    %10 = stablehlo.compare  GT, %8, %9,  FLOAT : (tensor<f64>, tensor<f64>) -> tensor<i1>
  ...
  func.func private @inputs() -> (tensor<complex<f64>> {mhlo.layout_mode = "default"}, tensor<complex<f64>> {mhlo.layout_mode = "default"}) {
    %cst = stablehlo.constant dense<(-0.16190613786021063,1.0031203742723367)> : tensor<complex<f64>>
    %cst_0 = stablehlo.constant dense<(-0.15782358560336177,0.97618358594854137)> : tensor<complex<f64>>
