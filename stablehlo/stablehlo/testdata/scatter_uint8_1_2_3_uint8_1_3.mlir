...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1x2x3xui8> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %0:2 = call @inputs() : () -> (tensor<1x2x3xui8>, tensor<1x3xui8>)
  ...
  func.func private @inputs() -> (tensor<1x2x3xui8> {mhlo.layout_mode = "default"}, tensor<1x3xui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[2, 4, 7], [1, 2, 1]]]> : tensor<1x2x3xui8>
    %c_0 = stablehlo.constant dense<[[6, 1, 3]]> : tensor<1x3xui8>
    return %c, %c_0 : tensor<1x2x3xui8>, tensor<1x3xui8>
  }
  func.func private @expected() -> (tensor<1x2x3xui8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[2, 4, 7], [6, 1, 3]]]> : tensor<1x2x3xui8>
