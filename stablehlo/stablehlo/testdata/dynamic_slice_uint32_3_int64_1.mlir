...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xui32> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    %0:2 = call @inputs() : () -> (tensor<3xui32>, tensor<1xi64>)
    ...
    %7 = stablehlo.dynamic_slice %0#0, %6, sizes = [1] : (tensor<3xui32>, tensor<i64>) -> tensor<1xui32>
    stablehlo.custom_call @check.expect_eq(%7, %1) {has_side_effect = true} : (tensor<1xui32>, tensor<1xui32>) -> ()
    return %7 : tensor<1xui32>
  }
  func.func private @inputs() -> (tensor<3xui32> {mhlo.layout_mode = "default"}, tensor<1xi64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<0> : tensor<3xui32>
    %c_0 = stablehlo.constant dense<1> : tensor<1xi64>
    return %c, %c_0 : tensor<3xui32>, tensor<1xi64>
  }
