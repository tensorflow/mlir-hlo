...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3xui64> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %2 = stablehlo.reduce(%0 init: %c) applies stablehlo.multiply across dimensions = [0] : (tensor<2x3xui64>, tensor<ui64>) -> tensor<3xui64>
  ...
  func.func private @inputs() -> (tensor<2x3xui64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[2, 0, 0], [1, 2, 3]]> : tensor<2x3xui64>
    return %c : tensor<2x3xui64>
  }
  func.func private @expected() -> (tensor<3xui64> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[2, 0, 0]> : tensor<3xui64>
