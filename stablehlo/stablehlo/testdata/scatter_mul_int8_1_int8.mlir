...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<1xi8> {jax.result_info = "", mhlo.layout_mode = "default"}) {
    ...
    %0:2 = call @inputs() : () -> (tensor<1xi8>, tensor<i8>)
  ...
  func.func private @inputs() -> (tensor<1xi8> {mhlo.layout_mode = "default"}, tensor<i8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<2> : tensor<1xi8>
    %c_0 = stablehlo.constant dense<-5> : tensor<i8>
    return %c, %c_0 : tensor<1xi8>, tensor<i8>
  }
  func.func private @expected() -> (tensor<1xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<-10> : tensor<1xi8>
