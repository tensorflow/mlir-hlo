...
module @jit_main attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  ...
  func.func private @inputs() -> (tensor<4x2x3xi8> {mhlo.layout_mode = "default"}, tensor<2xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[0, 0, 3], [0, -2, -4]], [[-2, 1, 2], [-1, 1, -1]], [[1, -2, 1], [0, -4, 2]], [[-5, 2, -3], [0, 0, 3]]]> : tensor<4x2x3xi8>
    %c_0 = stablehlo.constant dense<[-1, 2]> : tensor<2xi8>
    return %c, %c_0 : tensor<4x2x3xi8>, tensor<2xi8>
  }
  func.func private @expected() -> (tensor<4x2x3xi8> {mhlo.layout_mode = "default"}) {
    %c = stablehlo.constant dense<[[[0, 0, 3], [0, -2, -4]], [[-2, 1, 2], [-1, 1, -1]], [[1, -2, 1], [0, -4, 2]], [[-5, 2, 3], [0, 0, 6]]]> : tensor<4x2x3xi8>
