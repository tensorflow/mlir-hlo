...
LogicalResult verifyKnownCustomCalls(
  ...
  if (callTargetName == kCreateBufferCustomCallTarget) {
    if (!inputTypes.empty()) {
      return emitOptionalError(
          location, "CreateBuffer custom_call shouldn't have any inputs");
    }
    if (outputTypes.size() != 1) {
      return emitOptionalError(
          location, "CreateBuffer custom_call should have one result");
    }
    if (!isa<MemRefType>(outputTypes[0])) {
      return emitOptionalError(
          location, "CreateBuffer custom_call should have a memref result");
    }
    return success();
  }

  if (callTargetName == kPinCustomCallTarget) {
    if (inputTypes.size() != 1) {
      return emitOptionalError(location,
                               "Pin custom_call should have one input");
    }
    if (!isa<TensorType>(inputTypes[0])) {
      return emitOptionalError(location,
                               "Pin custom_call should have a tensor input");
    }
    if (outputTypes.size() != 1) {
      return emitOptionalError(location,
                               "Pin custom_call should have one output");
    }
    if (!isa<MemRefType>(outputTypes[0])) {
      return emitOptionalError(location,
                               "Pin custom_call should have a memref output");
    }

    if (failed(verifyCompatibleShape(inputTypes[0], outputTypes[0])) ||
        getElementTypeOrSelf(inputTypes[0]) !=
            getElementTypeOrSelf(outputTypes[0])) {
      return emitOptionalError(
          location,
  ...
  if (callTargetName == kUnpinCustomCallTarget) {
    if (inputTypes.size() != 1) {
      return emitOptionalError(location,
                               "Unpin custom_call should have one input");
    }
    if (!isa<MemRefType>(inputTypes[0])) {
      return emitOptionalError(location,
                               "Unpin custom_call should have a memref input");
    }
    if (outputTypes.size() != 1) {
      return emitOptionalError(location,
                               "Unpin custom_call should have one output");
    }
    if (!isa<TensorType>(outputTypes[0])) {
      return emitOptionalError(location,
                               "Unpin custom_call should have a tensor output");
    }
    ...
    if (failed(verifyCompatibleShape(inputTypes[0], outputTypes[0])) ||
        getElementTypeOrSelf(inputTypes[0]) !=
            getElementTypeOrSelf(outputTypes[0])) {
      return emitOptionalError(location,
                               "Unpin custom_call should have compatible input "
                               "and output types");
    }
  ...
  auto getOperandPartType = [&](int64_t operandIndex,
    ...
    Type operandPart = inputTypes[operandIndex];
  ...
  for (auto [outputTupleIndices, operandIndex, operandTupleIndices] :
       llvm::zip(outputTupleIndicesArray, operandIndicesArray,
                 operandTupleIndicesArray)) {
    ...
    if (!outputParts.insert(outputTupleIndices).second) {
      return emitOptionalError(
          location, "buffer output " + printIndices(outputTupleIndices) +
                        " is used in output_operand_alias more than once");
    }
    if (!operandParts.insert(std::make_pair(operandIndex, operandTupleIndices))
             .second) {
      return emitOptionalError(
          location, "buffer operand " + std::to_string(operandIndex) +
                        printIndices(operandTupleIndices) +
                        " is used in output_operand_alias more than once");
    }
  }

  Type resultType = outputTypes.size() == 1
                        ? outputTypes[0]
                        : TupleType::get(op->getContext(), outputTypes);
  if (failed(hlo::mapOverLeafTypes(
          ...
          [&](Type type, ArrayRef<int64_t> indices) -> LogicalResult {
            ...
            if (outputParts.find(indices) == outputParts.end()) {
              return emitOptionalError(
                  location, "buffer output " + printIndices(indices) +
